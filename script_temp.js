...
// Navigate to specific page with enhanced 3D effects
function navigateToPage(pageIndex) {
  ...
  currentPageEl.classList.add(
    direction === "next" ? "exiting" : "exiting-prev",
  );

  // Enhanced 3D page flip animation
  anime({
    ...
    begin: () => {
      // Add page depth preview effect
      bookPages.forEach((page, idx) => {
        if (idx === pageIndex + 1 && direction === "next") {
          page.classList.add("next-preview");
        } else if (idx === pageIndex - 1 && direction === "prev") {
          page.classList.add("prev-preview");
        }
      });
    },
    complete: () => {
      currentPageEl.classList.remove("active", "exiting", "exiting-prev");

      // Remove preview classes
      bookPages.forEach((page) => {
        page.classList.remove("next-preview", "prev-preview");
      });
    },
  });
  ...
  setTimeout(() => {
    ...
    // Animate new page entry with 3D effect
    anime({
      targets: nextPageEl,
      rotateY: [direction === "next" ? -90 : 90, 0],
      translateX: [direction === "next" ? "-100%" : "100%", 0],
      translateZ: [100, 0],
      scale: [0.8, 1],
      opacity: [0, 1],
      duration: 800,
...
// Smooth Page Scroll Within Container
